...
class Envira_Gallery_Import {
    ...
    public function import_remote_image( $src, $data, $item, $post_id, $id = 0, $stream_only = false ) {
        ...
        if ( is_wp_error( $stream ) || ! $type || strpos( $type, 'text/html' ) !== false ) {
            $this->errors[] = __( 'Could not retrieve a valid image from the URL ' . $new_image . '.', 'envira-gallery' );
            ...
            if ( $stream_only ) {
                return apply_filters( 'envira_gallery_remote_image_import_only_error', $stream );
            }
        } else {
            ...
            if ( ! empty( $mirror['error'] ) ) {
                ...
                $attachment = array(
                    'post_title'     => ( ( isset( $item['title'] ) && !empty( $item['title'] ) ) ? $item['title'] : basename( $new_image ) ), // Title
                    'post_mime_type' => $type,
