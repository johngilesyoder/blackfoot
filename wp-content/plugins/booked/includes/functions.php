				...
				for($x = 1; $x <= 7; $x++){
					...
					if (isset($day_name) && !in_array('prev-date',$classes)):
						...
						if (!empty($todays_defaults)):
							...
							$last_timeslot_in_day = key($todays_defaults);
							$end_of_day = explode('-',$last_timeslot_in_day);
							$end_of_day = $end_of_day[1];
							$time_right_now = date('Hi',$local_time);
							$this_date = $year.'-'.$month.'-'.$date;
							if ($time_right_now >= $end_of_day && $today_date == $this_date): $classes[] = 'prev-date'; endif;
						endif;	

					endif;
					
					$appointments_left = isset($total_full_count) ? $total_full_count - $appointments_count : 0;
				...
				*/
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$button_text = (!$available ? __('Unavailable','booked') : __('Book Appointment','booked'));
						$html = '<div class="timeslot bookedClearFix">';
							...
							$html .= '</span>';
							$html .= '<span class="timeslot-people"><button'.(!$available ? ' disabled' : '').' data-timeslot="'.$timeslot.'" data-date="'.$date.'" class="new-appt button"'.(!$spots_available ? ' disabled' : '').'><span class="button-timeslot">'.$timeslotText.'</span>'.apply_filters('booked_button_book_appointment', '<span class="button-text">'.$button_text.'</span></button></span>');
						$html .= '</div>';
					endif;
...
function booked_fe_calendar_date_square($date,$calendar_id = false){
	...
	echo '<span class="date'.($appointments_left && !in_array('prev-date',$classes) && !in_array('blur',$classes) ? ' tooltipster" title="'.sprintf( _n('%d time slot','%d time slots',$appointments_left,'booked'), $appointments_left) : '').'"><span class="number">'. $this_day .'</span></span>';
	echo '</td>';
